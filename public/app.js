...
async function submitForm(e, form) {
    ...
    const response = await fetchService.performPostHttpRequest(`http://localhost:3000/api/employee`, headers, jsonFormData); // Uses JSON Placeholder
    console.log(response);
    // 2.5 Inform user of result
    if(response)
        window.location = `/index.html?FirstName=${response.name}&LastName=${response.surname}&Email=${response.email}&id=${response.id}`;
    else
